...
A `lua` binary is the only dependency, and `./run.sh setup` fetches one into
`.tooling/` without sudo or a compiler. The test runner and linter are written
from scratch precisely so that neither luarocks, busted, nor luacheck is needed.
Python 3 drives the scraper, bundler and installer.
